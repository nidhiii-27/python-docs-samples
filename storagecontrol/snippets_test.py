...
@pytest.mark.skip(reason="Anywhere Cache creation is very long running")
def test_anywhere_cache_lifecycle(
    ...
    create_anywhere_cache.create_anywhere_cache(
        bucket_name=bucket_name, zone_name=zone_name
    )
    out, _ = capsys.readouterr()
    assert f"buckets/{bucket_name}/anywhereCaches/{zone_name}" in out

    # Test get anywhere cache
    get_anywhere_cache.get_anywhere_cache(bucket_name=bucket_name, zone_name=zone_name)
    out, _ = capsys.readouterr()
    assert f"buckets/{bucket_name}/anywhereCaches/{zone_name}" in out
